...
<html lang="en">
  <head>
    ...
    <style>
      ...
      .ok { color: #1a7f37; }
      .err { color: #b00020; }
      button { padding: .5rem 1rem; border-radius: 10px; border: 1px solid #ccc; cursor: pointer; }
  ...
  <body>
    <div class="card">
      ...
      <div class="meta">
        Model loaded:
        {% if model_loaded %}<span class="ok">True</span>{% else %}<span class="err">False</span>{% endif %}
        — Model path: <code>{{ model_path }}</code>
      </div>

      <form action="/predict-form" method="post">
        <label for="message"><strong>Enter text to classify:</strong></label><br>
        <textarea id="message" name="message" placeholder="Paste a news snippet here...">{{ request.form.get('message','') if request else '' }}</textarea><br>
      ...
      {% if prediction is not none %}
        ...
        Prediction:</strong> <span class="pred">{{ prediction }}</span>
        </div>
      {% endif %}
    </div>
  </body>
